...
struct Rect
{
    ...
    bool isValid() const { return height() > 0 && width() > 0; }
};
...
struct Window
{
    ...
    // Byte offsets in the source .opj file spanning this window's own
    // serialized data (header through its last layer) - [dataStartOffset,
    // dataEndOffset). Any embedded-page storage record whose own file
    // offset falls in this range is structurally nested inside this
    // window, which is how Origin itself associates a Note (or any other
    // embedded page) with its owning workbook - there is no separate
    // index or pointer field for it. 0/0 if unset (not populated for
    // window types where OriginAnyParser doesn't track it).
    long long dataStartOffset;
    long long dataEndOffset;
...
// see
// https://github.com/highperformancecoder/scidavis/commit/7c6e07dfad80dbe190af29ffa8a56c82a8aa9180
