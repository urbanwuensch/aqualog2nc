...
bool OriginAnyParser::readWindowElement()
{
    ...
    // Record this window's own byte range so callers can find embedded
    // pages (e.g. a Note's compressed storage record) that are
    // structurally nested inside it - see dataStartOffset's comment in
    // OriginObj.h for why this is how Origin itself associates them.
    {
        Origin::Window *w = nullptr;
        if (ispread != -1)
            w = &spreadSheets[ispread];
        else if (imatrix != -1)
            w = &matrixes[imatrix];
        else if (iexcel != -1)
            w = &excels[iexcel];
            ...
            w = &graphs[igraph];
        if (w) {
            w->dataStartOffset = wdh_start;
            w->dataEndOffset = curpos;
        }
    }
